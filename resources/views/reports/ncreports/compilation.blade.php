...
<img src="{{asset('imgs/user-guide/image-032.png')}}" alt="img">
<p>Right below the filters, you will see the total hours of all employees combined. Please note that
these numbers are only including individuals who have responded to the survey.</p>

<p>The next portion of the report is the entire taxonomy for all of the individuals compiled together.</p>
<img src="{{asset('imgs/user-guide/image-034.png')}}" alt="img">
<p>The attached picture above is what you will first see. You may click on either<strong> Legal Services</strong>, or <strong>Support Activities</strong> to expand the taxonomy of either. You will find that the taxonomy is listed in a hierarchical structure, and the further you expand the taxonomy, the more specific it will get.
...
<img src="{{asset('imgs/user-guide/image-036.png')}}" alt="img">
...
<img src="{{asset('imgs/user-guide/image-037.png')}}" alt="img">
<p>You will notice that on the far right side of each layer of the taxonomy, there is a blue icon with
people on it. This is called the<strong> Zoom Feature</strong>. The zoom feature allows for you to see each
...
<img src="{{asset('imgs/user-guide/image-038.png')}}" alt="img">
...
top right of your screen.</p>
   ...
   <!--  <script src="https://cdn.datatables.net/1.10.22/js/jquery.dataTables.min.js">
    ...
    <div class="container-fluid px-3 hideinhelppdf" style="background-color:#fff;"> 
    <div class="flex flex-wrap justify-between items-center cont-mtitle mb-4">
        ...
        <div class="d-flex py-0 py-md-2">
            <button class="revnor-btn mr-1" onclick="goBackList();" id="backBtn">Back</button>
    ...
    <div id="compilationDetailContent"  style="background-color:white; ">
        ...
        <div class="second_part pt-2 flex border-0" style="background-color: white;">
            <div>Filters-</div>
    ...
    <div id="compilationReportContent">
        ...
        <div class="third_part" style="background-color: white;border:1px solid #fff;"> 
            <div class="row mx-0 border-t border-b">
                <div class="col-md-12 stat_item">
                                ...
                                <table class="service_table table-responsive "> 
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="text-align: left;color:#82BD5E;width: 30%;">% of Total Cost</th>
                                            <th style="text-align: left;color:#367BC1;width: 30%;">% of Hours within selection</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        <tr onclick="getChildServiceData({{ $data['legal_id'] }}, {{ $data['legal_hours'] }}, {{ $data['legal_cost'] }}, 1, 'Legal Services', '');">
                                                @php
                                                    if ($data['total_hours'] > 0) {
                                                        $legal_percent = round($data['legal_hours'] / $data['total_hours'] * 100);
                                                    } else {
                                                        $legal_percent = 0;
                                                    }
                                                @endphp
                                        ...
                                        <tr onclick="getChildServiceData({{ $data['support_id'] }}, {{ $data['support_hours'] }}, {{ $data['support_cost'] }}, 1, '{{ $data['support_label'] }}', '');">
                                            @php
                                                if ($data['support_hours'] > 0) {
                                                    $support_percent = round($data['support_hours'] / $data['total_hours'] * 100);
                                                } else {
                                                    $support_percent = 0;
                                                }
                                            @endphp
                ...
                success: function (response) {
                    if (response.status && response.status == 400) {
                        ...
                        response.forEach(item => {
                                                    ...
                                                    <table class="service_table table-responsive">
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th style="text-align: right;width: 10%;">Hours</th>
                                                                <th style="text-align: left;color:#82BD5E;width: 30%;">% of Total Hours</th>
                                                                <th style="text-align: left;color:#367BC1;width: 30%;">% of Hours within selection</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>`;
                            ...
                            strHtml += `<tr>
                                            <td onclick="getChildServiceData(${item.question_id}, ${item.hours}, ${item.cost}, ${newdivNum}, '${item.question_desc}', '${title}');" style="text-align:right;">${item.question_desc}</td>
                                            <td style="text-align:right;width: 10%;">${(item.lead_codes !== null) ? item.lead_codes :''}</td>
                                            <td onclick="getChildServiceData(${item.question_id}, ${item.hours}, ${item.cost}, ${newdivNum}, '${item.question_desc}', '${title}');" style="text-align: right;">${numberFormatter.format(item.hours)}</td>
                                            <td onclick="getChildServiceData(${item.question_id}, ${item.hours}, ${item.cost}, ${newdivNum}, '${item.question_desc}', '${title}');">
                                                <div class="bar-graph flex items-center justify-start" style="width: 100%;">
                                            ...
                                            <td onclick="getChildServiceData(${item.question_id}, ${item.hours}, ${item.cost}, ${newdivNum}, '${item.question_desc}', '${title}');">
                                                <div class="bar-graph flex items-center justify-start" style="width: 100%;">
                                                    <div class="bg-revelation" style="width:calc(80% * ${item.selection_hours_pct} / 100);height:24px;"></div>
                                                    <span class="px-1">${item.selection_hours_pct}%</span>
                                                </div>
                                            </td>
                                            <td class="btn-detailList">
                        ...
                        $('.btn-detail-list').click(function() {
                            ...
                            $detailContainer.append(`<table class="table table-sm border-0 table-responsive" border="0" id="detailRespTable">
                                <thead>
                                    <tr>
                                        <th>Full Name</th>
                                        <th>Employee ID</th>
        ...
        */
        function generatePDF () {
            ...
            if ($('#compilationReportContent').css('display') == 'none') {
                ...
                pdfdoc.addImage(imgData_3, 'JPEG', 12, y, 190, imgHeight3); 
                y += imgHeight3;
        ...
        $('#excelBtn').click(function () {
            $.ajax({
                url: '{{ route("exportNCCompilationExcelData") }}',
                type: 'POST',
                data: {
                    "_token": "{{ csrf_token() }}",
                    "excelData": excelData,
